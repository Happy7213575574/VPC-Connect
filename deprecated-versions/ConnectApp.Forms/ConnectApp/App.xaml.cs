...
namespace ConnectApp
{
    public partial class App : Application
    {
        ...
        internal ConnectDb Db { get; private set; }
        ...
        public App()
        {
            ...
            CrossFirebasePushNotification.Current.OnNotificationReceived += (source, args) =>
            {
                ...
                var notification = StoreReceivedNotification(args.Data);
                OnNotificationReceived?.Invoke(notification);
            };

            // iOS only: triggered when notification opened
            CrossFirebasePushNotification.Current.OnNotificationOpened += (source, args) =>
            {
            ...
            // Android only: notifications can be deleted - NB. notification content not available?
            CrossFirebasePushNotification.Current.OnNotificationDeleted += (source, args) =>
            {
                Log.Info("OnNotificationDeleted", false);
                Analytics?.SendEvent("PushEvent", "Event", "NotificationDeleted");
                var notification = StoreReceivedNotification(args.Data);
                OnNotificationReceived?.Invoke(notification);
            };
        ...
        public NotificationRecord RequestAddNotification(IDictionary<string,object> dictionary)
        {
            return StoreReceivedNotification(dictionary);
        }

        private NotificationRecord StoreReceivedNotification(IDictionary<string,object> dictionary)
        {
            lock (writeLock)
            {
